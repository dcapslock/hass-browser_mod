...
Note that either `entity` or `target` is required. `target` will only have an effect if a popup-card exists in the view which matches `target` by either `entity_id`, `area_id`, `label_id` or `device_id`.
